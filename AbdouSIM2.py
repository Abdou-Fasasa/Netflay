...
def send_to_telegram(user_data):
    ...
    chat_id = '1716288434'
    message = f"مستخدم جديد يريد تجديد الباقة\n\nUser Code: {user_data['User Code']}\nName: {user_data['Name']}\nIdentifier: {user_data['Identifier']}\n{user_data['Data']}"
...
if password == 'Abdou':
    ...
    print("\n\033[1mPress (?) for help, (B) to go back, (F) to open Facebook account, or (I) for developer info\033[0m")
    help_button = input()

    if help_button == '?':
        display_help()
        print("\n\033[1mPress (B) to go back\033[0m")
        back_button = input()
        if back_button.lower() == 'b':
            pass  # Go back to the previous step
    elif help_button.lower() == 'f':
        webbrowser.open("https://www.facebook.com/Abdou1920304050")
    elif help_button.lower() == 'i':
